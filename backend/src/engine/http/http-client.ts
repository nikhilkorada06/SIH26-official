import axios, { AxiosError, AxiosRequestConfig, AxiosResponse, InternalAxiosRequestConfig, RawAxiosHeaders } from 'axios';
...
function sanitizeConfigForLogging(config: AxiosRequestConfig): Record<string, unknown> {
  const sanitized: Record<string, unknown> = { ...config };
  if (sanitized.headers) {
    const headers = { ...sanitized.headers } as Record<string, string>;
    for (const key of Object.keys(headers)) {
      const lowerKey = key.toLowerCase();
      if (lowerKey === 'authorization' || lowerKey === 'x-api-key' || lowerKey.includes('secret')) {
  ...
  constructor(config: Partial<HttpClientConfig> = {}) {
    ...
    this.axiosInstance.interceptors.request.use((requestConfig: InternalAxiosRequestConfig) => {
      ...
      if (breaker.getState() === 'OPEN') {
        throw EngineError.circuitOpen(cbKey);
      }
    ...
    for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
      try {
        ...
        if (response.status >= 400) {
          const engineError = EngineError.httpError(
            response.status,
            `HTTP ${response.status}: ${response.statusText}`,
            { url: axiosConfig.url, status: response.status }
          );
        ...
        if (error instanceof EngineError) {
          if (error.code === 'CIRCUIT_OPEN' || error.statusCode < 500) {
            throw error;
          }
        }
