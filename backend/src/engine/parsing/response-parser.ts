...
import { IntegrationDocument, ResponseConfig } from '../../models/Integration';
...
export class ResponseParser {
  static async parse(response: { data: unknown; headers: Record<string, string> }, integration: IntegrationDocument): Promise<ParsedResponse> {
    ...
    if (contentType.includes('xml') || contentType.includes('soap') || integration.protocol === 'SOAP') {
      ...
      // Strip namespace prefixes from keys for consistent path access
  ...
  private static parseXml(xmlString: string): Promise<unknown> {
    return new Promise((resolve, reject) => {
      parseString(xmlString, { explicitArray: false, trim: true }, (err: Error | null, result: unknown) => {
        if (err) {
          reject(EngineError.parse('Failed to parse XML response', { error: err.message }));
        } else {
          resolve(result);
        }
      });
