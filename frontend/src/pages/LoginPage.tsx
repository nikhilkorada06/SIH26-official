...
export const LoginPage: React.FC = () => {
  ...
  const submit=async(event:React.FormEvent)=>{event.preventDefault();if(!email||!password){setError('Please enter your registered email address and password.');return}try{setIsLoading(true);setError('');await login({email:email.trim(),password});navigate(destination,{replace:true})}catch(err:any){setError(err.response?.status===403&&err.response?.data?.requiresVerification?'Your account is not verified yet. Please complete registration OTP verification.':extractErrorMessage(err,'Invalid email address or password.'))}finally{setIsLoading(false)}};
